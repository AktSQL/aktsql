...
pub(in crate::view) fn query_sidebar_schema(state: &Akt) -> Element<'_, Message> {
...
pub(in crate::view) fn database_workbench_view(state: &Akt) -> Element<'_, Message> {
    let texts = i18n::texts(state.language());
    let form = state.connection_manager().form();
...
pub(in crate::view) fn detail_section_label(label: &'static str) -> Element<'static, Message> {
    text(label)
        .size(10)
        .wrapping(Wrapping::None)
        .style(theme::primary_text)
