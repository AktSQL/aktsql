...
pub(super) fn add_index_statement(
    ...
    let index_type = engine::schema::normalized_index_type(form.driver, draft.index_type());
    ...
    match form.driver {
        DatabaseDriver::MySql | DatabaseDriver::MariaDb | DatabaseDriver::TiDb => {
            ...
            let (prefix, using) = engine::schema::mysql_index_type_sql(&index_type);
        ...
        DatabaseDriver::PostgreSql | DatabaseDriver::CockroachDb => {
            let (unique, using) = engine::schema::postgres_index_type_sql(&index_type);
            Ok(format!(
        ...
        DatabaseDriver::Sqlite => Ok(format!(
            "CREATE {}INDEX {} ON {} ({});",
            engine::schema::sqlite_index_type_sql(&index_type),
            quote_plain_identifier(index),
            quote_plain_identifier(table),
            columns
        )),
        DatabaseDriver::SqlServer => Ok(format!(
            "CREATE {} INDEX {} ON {} ({});",
            engine::schema::sql_server_index_type_sql(&index_type),
        ...
        DatabaseDriver::Oracle => Ok(format!(
            "CREATE {}INDEX {} ON {} ({});",
            engine::schema::oracle_index_type_sql(&index_type),
            quote_plain_identifier(index),
            quote_plain_identifier(table),
            columns
        )),
        DatabaseDriver::MongoDb => Ok(format!(
            ...
            json_escape(table),
            engine::schema::mongodb_index_keys(columns, &index_type, json_escape),
            json_escape(index)
        )),
    }
}
...
pub(super) fn add_constraint_statement(
    ...
    let kind = engine::schema::normalized_constraint_type(form.driver, draft.constraint_kind());
...
pub fn last_table_part(table: &str) -> &str {
