...
pub(super) fn create_index_statement(
    ...
    let index_type = engine::schema::normalized_index_type(driver, index.index_type());
...
pub(super) fn mongodb_create_collection_commands(table: &str, draft: &CreateTableDraft) -> String {
    ...
    let indexes = draft
        ...
        .map(|index| {
            let index_type =
                engine::schema::normalized_index_type(DatabaseDriver::MongoDb, index.index_type());
            let keys =
                engine::schema::mongodb_index_keys(index.columns(), &index_type, json_escape);
            format!(
                "{{\"key\":{{{keys}}},\"name\":\"{}\"}}",
