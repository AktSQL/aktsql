...
本文从 `stitch_aktsql_database_management/` 下的原型反推出产品需求，并把需求切成小块，方便后续进入 OpenSpec 变更、任务排期和实现验收。
...
- Schema 管理必须覆盖新建表页面、字段 CRUD、索引管理、函数/过程索引和脚本导入导出。
...
| 编号 | 切片 | 优先级 | 主要来源 |
|---|---|---:|---|
| R01 | 产品命名与品牌基础 | P0 | 规格、logo、设计 |
| R02 | 应用主壳与固定布局 | P0 | 控制台、连接、设置 |
| R03 | 全局状态栏 | P0 | 控制台、连接、设置 |
| R04 | 导航结构 | P0 | 控制台、连接、设置 |
| R05 | 本地配置文件语义 | P0 | README, settings |
| R06 | 连接数据模型 | P1 | 连接 |
| R07 | 连接列表 | P1 | 连接 |
| R08 | 新建连接入口 | P1 | 连接、控制台 |
| R09 | 连接编辑表单 | P1 | 连接 |
| R10 | 连接测试 | P1 | 连接 |
| R11 | 保存并连接 | P1 | 连接 |
| R12 | 删除连接 | P1 | 连接 |
| R13 | 连接状态显示 | P1 | 控制台、连接 |
| R14 | SQL 编辑器基础 | P1 | 控制台 |
| R15 | SQL 执行按钮 | P1 | 控制台 |
| R16 | 查询执行反馈 | P1 | 控制台 |
| R17 | 结果表格基础 | P1 | 控制台 |
| R18 | 结果行数与耗时 | P1 | 控制台 |
| R19 | 数据库对象树 | P2 | 控制台 |
| R20 | 多 SQL 标签页 | P2 | 亮色控制台 |
| R21 | 结果面板标签 | P2 | 亮色控制台 |
| R22 | 查询消息 | P2 | 亮色控制台 |
| R23 | 执行计划入口 | P2 | 亮色控制台 |
| R24 | 结果搜索 | P2 | 控制台 |
...
| R36 | 快捷键档案 | P3 | 设置 |
| R37 | 配置应用与放弃 | P3 | 设置 |
| R38 | 暗色主题设计系统 | P0 | 设计、暗色控制台 |
| R39 | 亮色主题设计系统 | P3 | 亮色控制台 |
| R40 | 高对比红黑主题 | P3 | 设置 |
| R41 | SQL Server 2000 兼容提示 | P4 | 连接 |
| R42 | SSL 开关 | P4 | 连接 |
| R43 | SSH 隧道开关 | P4 | 连接 |
| R44 | 多数据库驱动策略 | P1-P4 | 规格、连接 |
| R45 | 搜索对象入口 | P2 | 亮色控制台 |
| R46 | 刷新入口 | P1 | 控制台、连接 |
| R47 | 过滤入口 | P2 | 控制台、连接 |
| R48 | 支持入口 | P4 | 控制台、设置 |
| R49 | 管理员头像与本地用户标识 | P4 | 控制台、设置 |
| R50 | 系统日志入口 | P2 | 页脚 |
| R51 | 性能状态指标 | P2 | 页脚 |
| R52 | 加密状态显示 | P4 | 亮色控制台 |
| R53 | 数据库版本显示 | P2 | 暗色控制台 |
| R54 | 编辑器光标位置显示 | P2 | 暗色控制台 |
| R55 | 命令面板入口 | P3 | 设置快捷键 |
| R56 | 移动端降级布局 | P4 | 设计 |
| R57 | 品牌 logo 资产 | P0 | logo |
| R58 | 表格状态徽标 | P1 | 控制台 |
| R59 | 微交互规范 | P2 | HTML scripts |
| R60 | 本地持久化开发者偏好 | P3 | 设置 |
| R61 | i18n 多语言体系 | P1 | 用户要求 |
| R62 | 完整连接参数分层 | P1 | 用户要求、Navicat 基线 |
| R63 | 数据库类型专属连接参数 | P1-P4 | 用户要求、Navicat 基线 |
| R64 | SQL 方言语法提示补全 | P1 | 用户要求 |
| R65 | 表名和字段名智能补全 | P1 | 用户要求 |
| R66 | 光暗模式快速切换 | P1 | 用户要求、设置 |
| R67 | Unix 哲学代码架构约束 | P0 | 用户要求 |
| R68 | 连接参数校验与缺省策略 | P1 | 用户要求 |
| R69 | SQL 美化与格式化 | P1 | 用户要求 |
| R70 | 新建表设计器 | P1 | 用户要求、Navicat 基线 |
| R71 | 表字段 CRUD | P1 | 用户要求、Navicat 基线 |
| R72 | 表索引管理 | P1 | 用户要求、Navicat 基线 |
| R73 | 函数/过程对象索引 | P2 | 用户要求、Navicat 基线 |
| R74 | 函数/过程查看与编辑 | P2 | 用户要求、Navicat 基线 |
| R75 | SQL 文件导入执行 | P2 | 用户要求、Navicat 基线 |
| R76 | 结构与数据导出 | P2 | 用户要求、Navicat 基线 |
| R77 | 表数据 CRUD | P2 | Navicat 基线 |
| R78 | 结构化变更确认 | P1 | 用户要求、Navicat 基线 |
| R79 | Navicat 类能力覆盖基线 | P0 | 用户要求 |
| R80 | 数据库级对象浏览 | P1 | 用户要求、PostgreSQL 基线 |
| R81 | 新建数据库 | P1 | 用户要求、Navicat 基线 |
| R82 | 删除表与清空表 | P1 | 用户要求、Navicat 基线 |
| R83 | 表分区查看 | P2 | 用户要求、Navicat 基线 |
| R84 | 整表结构摘要查看 | P1 | 用户要求、Navicat 基线 |
| R85 | 数据库关系图与 D2 展示 | P2 | 用户要求、Navicat 基线 |
| R86 | 稳定性与健壮性质量门禁 | P0 | 用户要求 |
...
需求：产品目标支持 SQL Server 2000+、MySQL、PostgreSQL、SQLite、Oracle。多数据库支持必须同时覆盖连接、元数据读取、SQL 方言、自动补全、执行结果、Schema 结构定义、数据库级对象、导入导出和错误映射。
...
- 每个驱动必须声明结构能力矩阵，例如新建表、修改字段、索引、外键、触发器、函数/过程、导出结构定义。
...
- 补全列表根据当前连接动态切换。
- 支持常见片段：查询、插入、更新、删除、关联、结构定义、结构修改、事务语句。
...
需求：必须提供工具化的新建表页面。用户可以通过图形界面创建表，配置字段、主键、索引、外键、约束、默认值、注释、引擎/表空间等参数，并直接执行建表操作。常见建表功能要覆盖高频数据库客户端工作流，人的审阅对象固定为结构化变更计划、校验、风险和执行结果。
...
- 保存直接执行数据库方言对应的建表操作；底层执行文本只作为内部驱动细节，不作为主交互路径或默认日志内容。
- 保存成功后刷新对象树并能打开新表。
...
- 修改已有表字段时能生成结构化变更计划，并由驱动执行对应动作。
- 删除字段需要确认，并提示可能造成数据丢失。
- 字段改名、类型变更、nullable 变更应在保存前的变更摘要或后续日志中明确展示。
- 字段新增和字段修改可以先进入未保存状态，按 Ctrl+S 保存并执行对应数据库操作。
...
需求：支持导出结构报告、对象定义包、查询结果和数据库/Schema 的结构归档。导出功能属于独立工具能力，不得嵌入新增表或修改表结构的主流程。
...
需求：Schema 变更类操作以参数表单为主。创建类操作在校验通过后直接执行，不展示、复制或要求用户审阅底层执行文本；破坏性或高风险操作必须在执行前确认，避免图形界面操作导致不可见的破坏性变更。
...
- 系统日志应记录执行的变更摘要、目标对象、耗时和失败原因；新增表和修改表结构日志默认展示结构化摘要。
- 执行成功和失败都记录到系统日志。
...
- 创建操作提交后直接执行，不跳转到查询编辑器，不展示底层执行文本。
...
- 重命名数据库和修改字符集/排序规则使用弹窗表单，提交后按数据库类型选择对应实现并直接执行；不跳转到查询编辑器。MySQL/MariaDB/TiDB 的数据库重命名必须执行时读取 `information_schema` 生成迁移计划，不能依赖左侧树是否已展开。
- 数据表或集合节点提供重命名操作，提交后按数据库类型执行原生重命名语义。
- 数据表或集合节点的 `SELECT ROWS` 直接查询结果，不把语句写入查询编辑器；默认每页 100 行，并在结果表格底部提供上一页/下一页。
- 数据表或集合节点的 `DESCRIBE TABLE` 直接展示只读表结构详情，按 Core、Storage/Encoding、Objects、Runtime 等父类分组，并展示字段、索引和结构摘要。
- 新增数据表使用结构化表单提交并直接执行，不进入查询编辑器；高级建表器后续扩展多字段、索引、约束和引擎选项。
- 数据表或集合节点的结构修改使用右侧工作台内的表设计器提交并直接执行，不使用弹窗；当前至少覆盖字段重命名、新增字段、新增索引、MySQL 兼容数据库字段移动。数据库不支持的物理字段移动必须直接显示不可用原因，不能退回到底层执行文本。

### R82 删除表与清空表

需求：表对象必须支持删除表和清空表，但这两类危险操作必须有强确认。非危险查看和编辑操作不得以底层执行文本作为主路径。

验收：

- 表对象菜单提供浏览行、查看结构、修改结构、重命名、删除表、清空表等自然动作。
- 删除表执行前显示确认弹窗，确认目标对象后才执行删除动作。
- 清空表执行前显示确认弹窗，确认目标对象后才执行清空动作或兼容语义。
...
需求：表详情页支持查看表分区信息，包括分区策略、分区字段、分区列表和分区结构摘要。
...
需求：表详情页必须支持查看整表结构摘要，包括字段、主键、索引、外键、注释、分区和数据库支持的表选项。底层执行文本不作为默认 GUI 内容。

验收：

- 表对象菜单或详情页提供“结构摘要”入口。
- 结构摘要能复制或保存为结构报告。
- 结构摘要应包含字段定义、主键、索引、外键、默认值、注释。
- 数据库支持时包含 engine、tablespace、partition、collation、storage options。
- 与 R76 导出能力复用同一结构元数据，避免两个地方输出不一致。
...
- 破坏性操作必须强确认、展示结构化风险摘要、写入日志。
- 网络断开、超时、权限不足、驱动不兼容、大结果集、元数据读取失败都必须有可恢复错误状态。
- UI 长任务必须可取消或至少显示进度，不能长期无响应。
- 配置损坏、缓存损坏、语言资源缺失、主题资源缺失时应用必须回退默认值。
- 核心模块需要单元测试，驱动适配需要集成测试，危险变更计划需要快照或等价测试。
- 发布前必须有基础 smoke test：启动、连接测试、执行查询、新建表结构化变更计划、查询编辑器格式化、对象树刷新。
...
7. 新建表页面：Fields、Indexes、Change Plan 三个核心区先跑通。
8. 表操作：支持查看整表结构摘要，支持删除表、清空表的强确认流程。
9. 结果表格：展示列、行、耗时、行数。
